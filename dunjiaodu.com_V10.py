...
def scrape_pages(start_page, end_page, port):
    ...
    for page in range(start_page, end_page + 1):
        ...
        links = driver.find_elements(
            By.XPATH, 
            "//a[starts-with(@href, '/dushu/') and substring(@href, string-length(@href) - 4) = '.html']"
        )
...
def scrape_article(driver, link_href,link_text):
    ...
    read_more_buttons = driver.find_elements(By.ID, 'input')
    if read_more_buttons:
        read_more_button = driver.find_element(By.ID, 'input')
        print('发现 "全文" 按钮')
    ...
    content_elements = driver.find_elements(By.ID, 'www_ecmsphp_com')
    if content_elements:
        content_element = driver.find_element(By.ID, 'www_ecmsphp_com')
    else:
        print('找不到文本内容 www_ecmsphp_com',link_text)
        return False
